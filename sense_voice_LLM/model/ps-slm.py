...
class slam_model_asr(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # <blk> embedding
        if self.voca_trans and not self.do_psd:
            self.blank_emb = nn.Parameter(
                torch.empty(1, 1536).normal_(mean=0.0, std=0.02)
            )
    ...
    def psd(
        ...
        返回:
            ...
            new_lens     : [B]   每句实际帧长
        """
        # print("psd: encoder_out shape:", encoder_out.shape)
        # print("psd: encoder_out_lens shape:", encoder_out_lens.shape)
        # print("psd: ctc_posterior shape:", ctc_posterior.shape)
    ...
    def forward(self,
        ...
        # print("forward: input_features shape:", input_features.shape)
        # print("forward: input_feature_length shape:", input_feature_length.shape)
        ...
        encoder_outs      = raw_encoder_out[:, 4:, :]          # [B, T-4, D]
        ...
        if self.ctc_posterior:
            ...
            if self.voca_trans == False: 
                ...
                if self.do_psd: # projector serves as a ctc head
                    projector_outs = self.encoder_projector(encoder_outs) 
                    projector_feature_length = encoder_out_lens // self.encoder_projector.k
                    ...
                    projector_outs = self.encoder_projector(encoder_outs) 
                    blank_downscale = 1e3
                    projector_outs[...,-1] -= math.log(blank_downscale)
                    projector_feature_length = encoder_out_lens // self.encoder_projector.k
                    llm_embedding = self.llm.get_input_embeddings()
                    embed_matrix = llm_embedding.weight
                    embed_matrix = torch.cat([embed_matrix, self.blank_emb], dim=0)
            ...
            if self.do_psd:
                encoder_outs, encoder_feature_length = self.psd(encoder_outs, encoder_out_lens, ctc_posterior,self.encoder.blank_id)
    ...
    def generate(self,
        ...
        if self.ctc_posterior:
            ...
            if self.do_psd:
                ...
                encoder_outs, encoder_feature_length = encoder_outs, encoder_out_lens
            projector_outs = self.encoder_projector(encoder_outs) 
            projector_feature_length = encoder_feature_length // self.encoder_projector.k    
