...
def setup_llm(train_config, model_config, **kwargs):
    ...
    if train_config.freeze_llm:   # 在finetune脚本中写了
...
class slam_model_asr(torch.nn.Module):
    ...
    def forward(self,
                ...
                sim_ctc: Optional[torch.Tensor] = None,      # [新增参数]
                sim_ctc_len: Optional[torch.Tensor] = None,  # [新增参数]
                ):
        ...
        if use_sim_mode:
            ...
            # 这些变量在模拟模式下不需要，设为 None
            raw_encoder_out = None
            raw_encoder_out_lens = None
            ctc_posterior = None
            encoder_out = None
        ...
        if self.ctc_posterior:
            ...
            if self.voca_trans == False: 
                ...
                # voca_trans 分支 (通常为 False，未做深度修改，仅做防报错处理)
                print("Vocabulary Transform is ready ...")
                # 如果在这种模式下开启了 use_sim_mode，我们需要确保 encoder_outs 有值
                if use_sim_mode:
                     encoder_outs = sim_ctc
                     encoder_feature_length = sim_ctc_len
                elif self.do_psd: # projector serves as a ctc head
                    pass # 原有逻辑会使用 encoder_outs，这里假设上面分支没覆盖到的话会报错，但正常流程不会进这
            ...
            # Raw feature 模式通常不支持模拟 CTC (因为格式不同)
            if self.do_psd:
