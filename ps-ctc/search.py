...
from utils import (add_sos_eos, log_add, add_whisper_tokens,
                                mask_to_bias)
from utils import remove_duplicates_and_blank
from utils import (make_pad_mask, mask_finished_preds,
...
def ctc_greedy_search(ctc_probs: torch.Tensor,
    ...
    for hyp in hyps:
        ...
        results.append(r)
        # PJ: we don't need DecodeResults to save here.
    return results
...
def ctc_prefix_beam_search(
    ...
    for i in range(batch_size):
        ...
        results.append(list(best))  # PJ: we only need token id
