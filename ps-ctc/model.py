...
class SenseVoiceCTC(nn.Module):
    def __init__(self, sensevoice, vocab_size, freeze_encoder=False):
        super().__init__()
        self.encoder = sensevoice
        if freeze_encoder:
            for param in self.encoder.parameters():
                param.requires_grad = False
            print("SenseVoice is frozen now ....")
        hidden_size = 512
        self.ctc_head = nn.Linear(hidden_size, vocab_size)
    ...
    def forward(self, input_features, input_feature_length):
        """
        input_features: (B, T, D)
        input_feature_length: (B, T)
        """
...
class SenseVoiceSmall(nn.Module):
    ...
    # pj: change here to load local dir
    @staticmethod
    def from_pretrained(model:str=None, device:str="cpu"):
        from funasr import AutoModel
        model, kwargs = AutoModel.build_model(model=model, device=device)
        return model, kwargs
