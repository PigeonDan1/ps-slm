...
from audiomentations.augmentations.pre_process import PitchPerturb
...
class TestPitchShift(unittest.TestCase):
    def test_apply_pitch_shift(self):
        ...
        sample_rate = 16000
        augmenter = Compose([PitchPerturb(min_semitones=-2, max_semitones=-1, p=1.0)])
    ...
    def test_apply_pitch_shift_multichannel(self):
        ...
        sample_rate = 16000
        augmenter = Compose([PitchPerturb(min_semitones=1, max_semitones=2, p=1.0)])
    ...
    def test_freeze_parameters(self):
        ...
        sample_rate = 16000
        augmenter = Compose([PitchPerturb(min_semitones=1, max_semitones=12, p=1.0)])

        first_samples = augmenter(samples_list=[samples], sample_rates_list=[sample_rate])
        ...
        second_samples = augmenter(samples_list=[samples], sample_rates_list=[sample_rate])
