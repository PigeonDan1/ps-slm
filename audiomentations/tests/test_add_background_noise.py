...
from audiomentations.augmentations.add_noise.pad_silence import PadSilence
...
class TestAddBackgroundNoise(unittest.TestCase):
    ...
    def test_add_2ch_background_noise(self):
        samples = np.sin(np.linspace(0, 440 * 2 * np.pi, 32000)).astype(np.float32)
        samples = np.stack((samples, samples), axis=0)
        sample_rate = 16000
        augmenter = Compose(
            [
                AddBackgroundNoise(
                    sounds_path=os.path.join(DEMO_DIR, "background_noises_2ch"),
                    min_snr_in_db=15,
                    max_snr_in_db=35,
                    p=1.0,
                )
        ...
        samples_out = augmenter(samples_list=[samples], sample_rates_list=[sample_rate])
        assert not np.allclose(samples, samples_out)
        self.assertEqual(samples_out.shape[0], 2)
        self.assertEqual(samples_out.dtype, np.float32)
    ...
    def test_strict_mode(self):
        ...
        augmenter_strict_mode = Compose(
            [
                pad_silence_time,
                add_background_noise
            ]
        )
        samples_strict_mode_out = augmenter_strict_mode(
            samples_list=[samples], sample_rates_list=[sample_rate])

        augmenter_nonstrict_mode = augmenter_strict_mode
        augmenter_nonstrict_mode.freeze_parameters()
        augmenter_nonstrict_mode.transforms[1].strict=False

        samples_nonstrict_mode_out = augmenter_nonstrict_mode(
            samples_list=[samples], sample_rates_list=[sample_rate])

        assert not np.allclose(samples_strict_mode_out, samples_nonstrict_mode_out)
        self.assertEqual(samples_strict_mode_out.dtype, np.float32)
