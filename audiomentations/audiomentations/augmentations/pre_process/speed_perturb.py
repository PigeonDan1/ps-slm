...
@add_transform('speed_perturb')
class SpeedPerturb(PreProcess):
    """Time stretch the signal without changing the pitch"""
    abbr = 'speed_perturb'
    supports_multichannel = True

    def __init__(self, min_rate=0.9, max_rate=1.1, leave_length_unchanged=False, p=1.0):
        super().__init__(p)
        assert min_rate > 0.1
        assert max_rate < 10
        ...
        self.choices = [x / 10 for x in range(int(min_rate*10), int(max_rate*10+1))]
    ...
    def randomize_parameters(self, samples, sample_rate, accumulate_meta=None):
        ...
        if self.parameters["should_apply"]:
            ...
            self.parameters["rate"] = np.random.choice(self.choices)
