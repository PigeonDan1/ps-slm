...
@add_transform('resample')
class Resample(PreProcess):
    """
    Resample signal using sox

    To do resampling to target_sample_rate and resampling back to original sample_rate
    """
    abbr = 'resample'
    ...
    def apply(self, samples, sample_rate):
        ...
        proc_downsample = subprocess.Popen(args=[
        ...
        proc_upsample = subprocess.Popen(args=[
                                'sox', '-', '-r', f'{sample_rate}',  '-t', 'wav', '-'
                        ],
                        stdin=subprocess.PIPE,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE)
        ...
        try:
            ...
            outs, log1 = proc_downsample.communicate(input=tmpfile_in.read(), timeout=30)
            outs, log2 = proc_upsample.communicate(input=outs, timeout=30)
        except subprocess.TimeoutExpired as e:
            proc_downsample.kill()
            proc_upsample.kill()
